...
Blockchain.prototype.addData = function(block) {
  $.checkArgument(isValidBlock(block));
...
Blockchain.prototype.checkValidBlock = function(block) {
  ...
  var coinbasePos = posToString(coinbase.pos);
  if (this.pixels[coinbasePos]) {
    throw new Error('Pixel in coinbase already mined');
  }
  var neighborList = neighbors(coinbase.pos);
  var adjacent = false;
...
Blockchain.prototype.confirm = function(block) {
  ...
  for (var i = 0; i < block.transactions.length; i++) {
    var tx = block.transactions[i];
    this.pixels[posToString(tx.pos)] = tx;
  }
